...
**Purpose & Goals:** Craft custom QR codes for widgets, lock-screen glance panels, and full-screen wallpapers. Offline F-Droid-friendly Android app.
...
- **In-app About (Module A):** `[AGENT]` scaffold `AboutScreen` with version, quiet **Donate via Venmo** under Settings → About (never TopAppBar), and Continuum-method launch prompts (`docs/features/donations-updates.md`, setup walkthrough `docs/help/DONATIONS.md`). Persist `installed_artifact_format` (`apk`) on first run. Compare `{prefix}-X.Y.Z-foss.apk` filenames from GitHub assets, not git tags. Install opens the asset URL (fallback: release page). No Play Core or proprietary in-app update SDKs.
...
- **In-app About (Module B):** `[AGENT]` scaffold About panel with quiet donate + `productUpdate.ts` / `runAppUpdates.ts` (daily GitHub installer check, once-per-version donate note). PWA `applyPwaUpdate()` stays About-only (`skipWaiting`, single `location.reload()` with restart guard). Installed format is fixed `pwa`.
...
- **In-App About Screen & App Update Checker (product UI — not GitHub About, not template checker):**
  - **`[AGENT]`** Scaffold an accessible **About** screen reachable from Settings → About (and primary About navigation): app name, version, license/project URL, quiet **Donate via Venmo** (Android: not in the titlebar). See `docs/help/DONATIONS.md` for GitHub Sponsors and international methods.
  - **Update check:** once per 24 hours, GitHub Releases API with User-Agent + 10s timeout. Compare product installer filenames, not git/template tags. Failed fetch stays silent.
  - **Update prompt:** **Install** | **Later**. Install opens the asset URL (fallback: release page). Later silences that version. Never mix donate onto this dialog.
  - **Donate reminder:** first run records version (no popup). After a version change, one optional note. Either button records “seen this version.”
  - **Installed format detection (first run):** detect and persist `installed_artifact_format`; **never switch formats** on update.
  - **Donations:** default public Venmo link in `donations.json`; `[HUMAN]` may add GitHub Sponsors / Liberapay / Open Collective / PayPal / etc. at init (`docs/help/DONATIONS.md`); external browser only; no donation tracking.
  - **Privacy:** document release-check calls in `docs/PRIVACY.md` (device-local `last_check_at`, `last_seen_version`, `dismissed_version`; no PII).
