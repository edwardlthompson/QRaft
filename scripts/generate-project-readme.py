...
def stack_badges(product: dict) -> str:
    ...
    lines = []
    for stack in stacks:
        color = colors.get(str(stack), primary)
        lines.append(
...
def license_fields(root: Path) -> tuple[str, str]:
    cfg = root / "bootstrap.config.json"
    license_id = "MIT"
    if cfg.is_file():
        license_id = str(json.loads(cfg.read_text(encoding="utf-8")).get("license") or "MIT")
